...
@section('content')
    <main itemscope itemtype="http://schema.org/WebPage">
        <!-- Header -->
        <header class="intro-header" itemscope itemtype="https://schema.org/WebSite">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="intro-message">
                            <h1 itemprop="headline">{{ config('app.name') }}</h1>
                            <h3 itemprop="description">{{ config('app.slogan') }}</h3>
                            <hr class="intro-divider">
                            <ul class="list-inline intro-social-buttons">
                                @foreach(config('social.brand') as $brand)
                                    @if(!empty($brand['url']))
                                        <li class="list-inline-item">
                                            <a rel="external" itemprop="sameAs" href="{{ $brand['url'] }}" target="_blank" class="btn btn-secondary btn-lg"><i class="fa {{ $brand['css'] }} fa-fw"></i> <span class="network-name">{{ $brand['name'] }}</span></a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
                ...
                <div class="row">
                    @forelse($tutorials as $tutorial)
                        <article class="col-md-4">
                            <div class="card">
                                <a title="{{ $tutorial->title }}" href="{{ action('ContentCtrl@tutorial', $tutorial->slug) }}">
                                    @widget('Img', [
                                        'id' => $tutorial->id,
            ...
            <div class="container">
                ...
                <div class="row">
                    @forelse($contents as $content)
                        <div class="col-md-4">
                            <div class="card">
                                <a title="{{ $content->title }}" href="{{ action('ContentCtrl@content', $content->slug) }}">
                                    @widget('Img', [
                                        'id' => $content->id,
