    ...
    <main>
                                ...
                                @foreach(config('social.brand') as $brand)
                                    @if(!empty($brand['url']))
                                        <li class="list-inline-item">
                                            <a rel="external" href="{{ $brand['url'] }}" target="_blank" class="btn btn-secondary btn-lg"><i class="fa {{ $brand['css'] }} fa-fw"></i> <span class="network-name">{{ $brand['name'] }}</span></a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
        ...
        <section>
            <div class="container">
                ...
                <div class="clearfix"></div>
                <div class="row">
                    @forelse($tutorials as $i => $tutorial)
                        <article class="col-md-4">
                            <div class="card">
                                <a title="{{ $tutorial->title }}" href="{{ action('ContentCtrl@tutorial', $tutorial->slug) }}">
                                    ...
                                    <span class="card-text">{{ str_limit($tutorial->title, 40) }}</span>
                                </a>
                            </div>
                        </article>
                    @empty
